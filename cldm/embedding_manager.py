...
def get_recog_emb(encoder, img_list):
    ...
    _img_list = [(img.repeat(1, repeat, 1, 1)*255)[0] for img in img_list]
    encoder.predictor.eval()
    # pred_imglist 现在返回 (ctc, (visual, neck))
    # 字形编码(recog)仍然只使用 neck 特征
    _, (_, preds_neck) = encoder.pred_imglist(_img_list, show_debug=False)
    return preds_neck
...
def get_style_emb(encoder, img_list):
    ...
    # encoder.predictor.train()
    # pred_imglist 现在返回 (ctc, (visual, neck))
    # 风格编码(style)需要返回完整的特征元组
    _, feature_tuple = encoder.pred_imglist(_img_list, show_debug=False)
    return feature_tuple # 返回 (visual, neck) 元组
...
class EmbeddingManager(nn.Module):
    def __init__(
            ...
            use_calligraphy_style=False,  # <-- 1. 接受新参数
            add_color=False,
        ...
        if add_style_ocr:
            self.use_calligraphy_style = use_calligraphy_style  # <-- 2. 保存开关状态
            if self.style_ocr_trainable:
                self.font_predictor = create_predictor()
                args = edict()
                args.rec_image_shape = "3, 48, 320"
            ...
            # --- 3. 定义 *原始* 投影层 (用于 "关闭" 状态) ---
            # 它只处理 Neck 特征 (40*64 = 2560)
            self.style_proj = nn.Sequential(
                                zero_module(linear(40*64, token_dim)),
                                nn.LayerNorm(token_dim)
                                )

            # --- 4. 定义 *新的* 融合模块 (用于 "开启" 状态) ---
            if self.use_calligraphy_style:
                # (这些模块只有在开关为 True 时才会被创建)
                # 维度
                backbone_C = 512  # 修复: MobileNetV1Enhance scale=0.5 输出 512 通道
                neck_feat_dim = 40 * 64 # Neck 展平后维度
                proj_hidden_dim = 512 # 自定义中间维度

                # 分支 1: 处理 Backbone
                self.style_visual_processor = nn.Sequential(
                    nn.AdaptiveAvgPool2d((1, 1)),
                    nn.Flatten(),
                    linear(backbone_C, proj_hidden_dim)
                )

                # 分支 2: 处理 Neck
                self.style_sequence_processor = nn.Sequential(
                    nn.Flatten(start_dim=1),
                    linear(neck_feat_dim, proj_hidden_dim)
                )

                # 融合与共享投影层:
                self.style_shared_projector = nn.Sequential(
                    nn.SiLU(),
                    nn.LayerNorm(proj_hidden_dim * 2), # 512 + 512 = 1024
                    zero_module(linear(proj_hidden_dim * 2, token_dim)),
                    nn.LayerNorm(token_dim)
                )
    ...
    def encode_text(self, text_info):
        ...
        if len(gline_list) > 0:
            ...
            #     enc_glyph = enc_glyph+enc_style
            if self.add_style_ocr:
                # get_style_emb 现在返回一个 (visual, sequence) 元组
                style_emb_tuple = self.get_style_emb(style_list)
                
                if self.use_calligraphy_style:
                    # --- 分支 1: "开启" 状态 (新的融合逻辑) ---
                    style_emb_visual = style_emb_tuple[0]    # [B, 256, H, W]
                    style_emb_sequence = style_emb_tuple[1]  # [B, 40, 64]

                    # 分别处理
                    visual_vec = self.style_visual_processor(style_emb_visual)
                    sequence_vec = self.style_sequence_processor(style_emb_sequence)

                    # 融合
                    fused_vec = torch.cat([visual_vec, sequence_vec], dim=1) # -> [B, 1024]

                    # 投影
                    enc_style = self.style_shared_projector(fused_vec) # -> [B, token_dim]
                
                else:
                    # --- 分支 2: "关闭" 状态 (原始逻辑) ---
                    # 原始逻辑只使用 Neck 特征
                    style_emb_sequence = style_emb_tuple[1] # [B, 40, 64]
                    enc_style = self.style_proj(style_emb_sequence.reshape(style_emb_sequence.shape[0], -1))
