...
class TextRecognizer(object):
    ...
    def pred_imglist(self, img_list, show_debug=False):
        ...
        preds_all = [None] * img_num
        preds_neck_all = [None] * img_num
        preds_visual_all = [None] * img_num  # <-- 新增 visual 特征列表
        for beg_img_no in range(0, img_num, batch_num):
            end_img_no = min(img_num, beg_img_no + batch_num)
            norm_img_batch = []
            ...
            if self.is_onnx:
                ...
                preds['ctc'] = torch.from_numpy(outputs[0])
                preds['ctc_neck'] = [torch.zeros(1)] * img_num
                preds['visual_features'] = [torch.zeros(1)] * img_num # 假设的 onnx fallback
            else:
                preds = self.predictor(norm_img_batch)
            for rno in range(preds['ctc'].shape[0]):
                preds_all[indices[beg_img_no + rno]] = preds['ctc'][rno]
                preds_neck_all[indices[beg_img_no + rno]] = preds['ctc_neck'][rno]
                preds_visual_all[indices[beg_img_no + rno]] = preds['visual_features'][rno] # <-- 填充 visual 特征

        # v-- 修改返回格式：(ctc, (visual, neck))
        return torch.stack(preds_all, dim=0), (torch.stack(preds_visual_all, dim=0), torch.stack(preds_neck_all, dim=0))
