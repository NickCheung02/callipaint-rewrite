...
class RecModel(nn.Module):
    ...
    def forward(self, x, targets=None):
        visual_features = self.backbone(x)     # 1. 捕捉 backbone 的输出
        x = self.neck(visual_features)         # 2. 将其送入 neck
        # 修复: self.head (CTCHead) 不接受 'targets' 参数，所以我们只传递 x
        x = self.head(x) 
        x['visual_features'] = visual_features # 3. 将 backbone 特征添加到返回字典中
        return x
