...
###修改2结束####################################################################
    ...
    def __getitem__(self, item):
        ...
        # --- NEW: 为阶段一和阶段二加载所有提示词 ---
        # 1. 设置一个回退，以防数据中缺少某些键
        #    (使用 'caption' 作为旧格式的 'full_prompt' 回退)
        default_caption = cur_item.get('caption', '') 
        full_prompt = cur_item.get('full_prompt', default_caption)
        
        # 2. 为 'img_caption' (旧键) 和 'full_prompt' (新键) 应用截断
        if self.trunc_cap > 0 and self.training_stage == 2: # 阶段二才截断
             full_prompt = truncate_string(full_prompt+'. ', max_words=self.trunc_cap)
        
        # 3. 将所有提示词加载到 item_dict 中
        item_dict['img_caption'] = full_prompt  # 旧键，用于兼容
        item_dict['full_prompt'] = full_prompt
        # (如果新键不存在，统一回退到 full_prompt)
        item_dict['element_prompt'] = cur_item.get('element_prompt', full_prompt)
        item_dict['mood_prompt'] = cur_item.get('mood_prompt', full_prompt)
        item_dict['style_prompt'] = cur_item.get('style_prompt', full_prompt)
        # --- NEW: 修改结束 ---
        ...
        # --- 以下是阶段二的逻辑 ---
        # (我们已经加载了 'img' 和 'img_caption' / 'full_prompt' 等)
        ...
        # (不再需要: item_dict = {} )
        # (不再需要: item_dict['img_caption'] = cur_item['caption'] )
